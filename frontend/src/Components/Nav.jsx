...
function Nav({ show, toggle }) {
    ...
    const activeLink = CSS + " bg-primary  text-white rounded-md pr-4"
    const inactiveLink = CSS + " text-black"

    const IconCSS = " w-6 h-6"
    const activeIcon = IconCSS + "bg-primary  text-white"
    ...
    return (
        <aside className={(show ? " left-0 " : " -left-full ") + 'top-0 p-4 fixed w-full h-full md:static md:w-auto transition-all gap-2 flex bg-white flex-col z-50'}>
